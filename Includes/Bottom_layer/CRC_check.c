...
uint16_t crc16_get_checksum(const uint8_t *pchMessage, uint32_t dwLength,
  ...
  while (dwLength--) {
    ...
    wCRC = (uint16_t)(wCRC >> 8) ^
           CRC16_TABLE[((uint16_t)wCRC ^ (uint16_t)ch_data) & 0x00FF];
  }
...
int crc16_verify_checksum(const uint8_t *pchMessage, uint32_t dwLength) {
  ...
  w_expected = crc16_get_checksum(pchMessage, dwLength - 2, CRC16_INIT);
...
void crc16_append_checksum(uint8_t *pchMessage, uint32_t dwLength) {
  ...
  w_crc = crc16_get_checksum(pchMessage, dwLength - 2, CRC16_INIT);
