...
/**
 * @brief  偷看指定偏移的未读数据（单字节），不移动读指针
 * @param  control: 环形缓冲区句柄
 * @param  byte:    输出字节的指针
 * @param  index:   未读数据的偏移索引（0 = 第一个未读元素）
 * @retval RING_BUFFER_SUCCESS / RING_BUFFER_ERROR
 */
uint8_t RingBuffer_PeekByte(RingBuffer *control, uint8_t *byte,
                            uint32_t index) {
  if (!ringBuffer_IsConfigValid(control) || (byte == NULL)) {
    return RING_BUFFER_ERROR;
  }

  uint32_t unread_cnt = RingBuffer_GetUnreadCount(control);
  if (index >= unread_cnt) {
    return RING_BUFFER_ERROR;
  }

  uint32_t read_pos =
      ringBuffer_WrapIndex(control->read_index + index, control->max_elements);

  *byte = control->buffer[read_pos];
