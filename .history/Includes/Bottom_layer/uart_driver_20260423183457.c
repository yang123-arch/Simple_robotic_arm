...
uint8_t uart1_dma_rx_buffer_0
    [UART1_DMA_RX_BUFFER_SIZE]; // UART1 DMA接收缓冲区0，定义为128字节
uint8_t uart1_dma_rx_buffer_1[UART1_DMA_RX_BUFFER_SIZE]; //
...
static HAL_StatusTypeDef UartDmaIdleRx_Init(UartDmaIdleRx_t *ctx,
  ...
  // 初始化底层硬件相关
  ctx->huart = huart;
  DoubleBuffer_Template.init_Buffer(&ctx->uart_data, buffer0, buffer1,
                                    buffer_size);
...
static inline void UART_DMA_IDLE_IRQ_HANDLER(UartDmaIdleRx_t *ctx,
  ...
  if ((ctx) != NULL && (huart) == (ctx)->huart) {
    ...
    UartDmaIdleRx_HandleEventFromISR((ctx), (size), &xHigherPriorityTaskWoken,
                                     parser);
...
void init_uart_dma(void) {
  UartDmaIdleRx_Init(&uart1_dma_idle_rx, &huart1, uart1_dma_rx_buffer_0,
                     uart1_dma_rx_buffer_1, UART1_DMA_RX_BUFFER_SIZE,
                     (TaskHandle_t)UART_SOLVEHandle);
}
...
void HAL_UARTEx_RxEventCallback(UART_HandleTypeDef *huart, uint16_t Size) {
  ...
  UART_DMA_IDLE_IRQ_HANDLER(&uart1_dma_idle_rx, huart, Size, &parser_uart1);
}