...
typedef struct {
  ...
  uint8_t (*get_data_from_uart)(
      UartDmaIdleRx_t *ctx, UART_HandleTypeDef *huart, uint16_t buffer_size,
      TaskHandle_t
          task_to_notify); // 获取数据函数指针，指向UartDmaIdleRx_GetEvent函数，用于获取接收事件，如果有事件就绪，则返回对应缓冲区指针、数据长度和事件类型，并清除事件就绪状态
...
#define UART_DMA_IDLE_IRQ_HANDLER(ctx, huart, size)                            \
  do {                                                                         \
    if ((ctx) != NULL && (huart) == (ctx)->huart) {                            \
      BaseType_t xHigherPriorityTaskWoken = pdFALSE;                           \
      uart_dma_idle_ops.handle_event_from_isr((ctx), (size),                   \
