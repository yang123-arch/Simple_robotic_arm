...
static inline uint32_t ringBuffer_WrapIndex(uint32_t index,
  ...
  } //当max_elements为2的幂时，最后几位都为1，直接&可以实现模运算，效率更高。
    //相当于只保留低于该幂次的部分，直接丢弃高位。
...
uint32_t ringBuffer_GetUnreadCount(uint32_t read_index, uint32_t write_index,
  ...
  unread = (write_index + max_elements - read_index) % max_elements;
...
void RingBuffer_Init(RingBuffer_Control *control, uint64_t element_size,
                     uint32_t max_elements, uint8_t *buffer) {
