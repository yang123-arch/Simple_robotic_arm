...
void UartTaskParser_Init(UartTaskParser_t *parser) {
  ...
  (void)RingBuffer_Init(&uart_rx_ring_buffer, UART_TASK_RING_BUFFER_SIZE, 1,
                        uart_rx_buffer);
  parser->ring = uart_rx_ring_buffer;
   ...
   */
  UartTaskParseStatus_t UartTaskParser_ParseFrame(
      UartTaskParser_t * parser, UartFeedbackFrame_t * out_frame) {
    uint8_t *frame_ptr = (uint8_t *)out_frame;
    uint16_t i;
    uint16_t idx;
