...
UartTaskParseStatus_t UartTaskParser_ParseFrame(UartTaskParser_t *parser) {
  ...
  while (RingBuffer_GetFreeCount(&parser->ring_buffer) >= UART_FEEDBACK_FRAME) {
    ...
    if (byte != UART_FEEDBACK_FRAME_SOF0) {
      index++;
      RingBuffer_Read(&parser->ring_buffer, 1, &byte);
      continue; // 第一个起始符不匹配，丢弃一个字节继续寻找
    }
    RingBuffer_PeekByte(&parser->ring_buffer, &byte, index + 1);
    if (byte != UART_FEEDBACK_FRAME_SOF1) {
      index++;
      RingBuffer_Read(&parser->ring_buffer, 2,
                      &byte); // 第二个起始符不匹配，丢弃两个字节继续寻找
      continue;               // 第二个起始符不匹配，继续寻找
    }
    // 已找到完整帧头，尝试读取整帧
    if (RingBuffer_GetUnreadCount(&parser->ring_buffer) <
        index + UART_FEEDBACK_FRAME) {
      return UART_TASK_PARSE_NONE; // 数据不足，等待更多数据到来
    }
    // 读取完整帧数据到临时缓冲区
    uint8_t temp_frame[UART_FEEDBACK_FRAME];
    RingBuffer_Read(&parser->ring_buffer, UART_FEEDBACK_FRAME, temp_frame);
  }
  return UART_TASK_PARSE_NEED_MORE_DATA; // 没有找到有效帧
}
  ...
  for (;;) {
    ...
    do {
      ...
      if (status == UART_TASK_PARSE_OK) {
        DoubleBuffer_Template.Buffer_event(&uart1_double_buffer);
      }

      /* 其他状态（如 NEED_MORE_DATA）可忽略，继续循环 */
    } while (status == UART_TASK_PARSE_OK); /* NONE 表示无数据可取 */
  }
}