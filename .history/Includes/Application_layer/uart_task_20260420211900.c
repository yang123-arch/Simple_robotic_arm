...
static uint8_t UartTaskRing_Peek(const UartTaskParser_t *parser, uint16_t offset,
                                 uint8_t *byte) {
  uint32_t index;
...
static uint8_t UartTaskFrame_CalcChecksum(const UartTaskParser_t *parser,
  ...
  uint8_t value = 0U;
  uint16_t i;
  uint16_t bytes_to_xor = (uint16_t)payload_length + 2U; /* LEN + CMD + PAYLOAD */
...
UartTaskParseStatus_t UartTaskParser_ParseFrame(UartTaskParser_t *parser,
  ...
  for (;;) {
    ...
    if (payload_length > UART_FEEDBACK_FRAME_MAX_PAYLOAD_LEN) {
      UartTaskRing_Discard(parser, 1U);
      parser->dropped_bytes++;
      continue;
    }

    frame_size = (uint16_t)payload_length + UART_FEEDBACK_FRAME_FIXED_OVERHEAD;
    if (parser->count < frame_size) {
      return UART_TASK_PARSE_NEED_MORE_DATA;
    }

    (void)UartTaskRing_Peek(parser, 3U, &command_id);
    calculated_checksum = UartTaskFrame_CalcChecksum(parser, payload_length);
    (void)UartTaskRing_Peek(parser, (uint16_t)(frame_size - 1U),
                            &expected_checksum);
    ...
    for (i = 0U; i < payload_length; i++) {
      (void)UartTaskRing_Peek(parser, (uint16_t)(4U + i), &out_frame->payload[i]);
    }
    for (; i < UART_FEEDBACK_FRAME_MAX_PAYLOAD_LEN; i++) {
      out_frame->payload[i] = 0U;
    }

    UartTaskRing_Discard(parser, frame_size);
