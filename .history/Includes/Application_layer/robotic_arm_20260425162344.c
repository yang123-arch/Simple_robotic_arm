...
void Arm_Control_Single(float angle_target, PID_CONTROL *pid_yaw_PD,
                        PID_CONTROL *pid_yaw_PI, Motor *status) {
  ...
  if (fabs(angle_error) < 5.0f) {
    pid_yaw_PI.error_total = 0; // 清积分防饱和
    status->current_TX = 0;
    control_Servo_Yaw(0);
    return 0;
  }

  float equivalent_target = yaw_angle_actual + angle_error;

  float yaw_output_PD =
      PID_control(yaw_angle_actual, equivalent_target, &pid_yaw_PD);
  ce_shi = yaw_output_PD;
  float speed_target;
  if (fabs(Compensation_speed) < 1.0f) {
    speed_target = yaw_output_PD + Compensation_speed * 8.0f;
  } else {
    speed_target = yaw_output_PD + Compensation_speed * 1.5f;
  }

  float yaw_output_PI = PID_control(Yaw->Speed_RPM, speed_target, &pid_yaw_PI);
  ce_shi = yaw_output_PI;
  ...
  Yaw->Servo_current = (int16_t)yaw_output_PI;
